        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar print:p-4 print:space-y-6">
          ...
          <div className="border-b-2 border-slate-800 pb-4 mb-6 flex justify-between items-center">
            <div className="flex items-center gap-3.5">
              {printHeaderLogo ? (
                <img src={printHeaderLogo} alt="شعار السنتر" className="w-14 h-14 object-contain rounded-xl border border-slate-200 bg-slate-50 shrink-0" />
              ) : (
                <div className="w-12 h-12 bg-amber-500/10 border-2 border-amber-600 rounded-xl flex items-center justify-center text-amber-800 font-extrabold text-xl shrink-0">
                  {printHeaderTitle ? printHeaderTitle.charAt(0) : 'س'}
                </div>
              )}
              <div>
                <h1 className="text-lg font-extrabold text-slate-900 leading-tight">{printHeaderTitle}</h1>
                <p className="text-xs text-slate-600 font-medium">{printHeaderSubtitle}</p>
                {printHeaderContact && <p className="text-[10px] text-slate-500 font-sans">{printHeaderContact}</p>}
              </div>
            </div>

            <div className="text-center px-3 py-1.5 bg-slate-50 border border-slate-300 rounded-xl shrink-0">
              <span className="text-xs font-bold text-slate-800 block">تقرير طالب رسمي</span>
